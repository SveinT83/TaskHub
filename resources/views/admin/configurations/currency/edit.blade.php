...
@section('pageHeader')
    <x-page-header pageHeaderTitle="{{ __('core.currency.edit_currency') }}">
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5 class="card-title mb-0">{{ __('core.currency.edit_currency') }}: {{ $currency->name }} ({{ $currency->code }})</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('currency.update', $currency) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            @enderror
                            <div class="form-text">
                                {{ __('core.currency.exchange_rate_help') }}
                                @if($currency->rate_updated_at)
                                    <br><small class="text-muted">{{ __('core.currency.last_updated') }}: {{ $currency->rate_updated_at->format('Y-m-d H:i') }}</small>
                                @endif
                            </div>
                        </div>
